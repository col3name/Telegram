...
//        colorMatrix.setSaturation(2f);
        ...
        boolean ok = (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP && SharedConfig.getDevicePerformanceClass() >= SharedConfig.PERFORMANCE_CLASS_HIGH);
        impl = new CPUImpl(!ok);
    ...
    private int getBottomExtra() {
        return (int) (AndroidUtilities.dp(BUTTONS_FADE_DP + 64 + 8 * 2) / getScaleY());
    }
    ...
    public void onSizeChanged(int w, int h) {
        ...
        fadePaint.setShader(new LinearGradient(w / 2f, AndroidUtilities.dp(BUTTONS_FADE_DP) / getScaleY(), w / 2f, 0, new int[] {0x00FFFFFF, 0xFFFFFFFF}, new float[]{0f, 1f}, Shader.TileMode.CLAMP));
        mView.invalidate();
    }
    ...
    private final class CPUImpl implements Impl {
        ...
        public void draw(Drawer drawer, Canvas canvas) {
            ...
            AndroidUtilities.rectTmp.set(0, 0, mView.getWidth(), AndroidUtilities.lerp(y + AndroidUtilities.dp(BUTTONS_FADE_DP) / getScaleY() - 1, mView.getHeight(), 1f - progress));
            canvas.saveLayer(AndroidUtilities.rectTmp, null, Canvas.ALL_SAVE_FLAG);
            drawer.draw(canvas);
            float off = AndroidUtilities.dp(BUTTONS_FADE_DP) / getScaleY() * (1f - progress);
            canvas.translate(0, y + off);
            canvas.drawRect(0, 0, mView.getWidth(), AndroidUtilities.dp(BUTTONS_FADE_DP) / getScaleY() + 1, fadePaint);
            canvas.restore();
        }
    ...
    private final class GPUImpl implements Impl {
        ...
        public void draw(Drawer drawer, Canvas canvas) {
            ...
            // Draw buffer with fade
            canvas.saveLayer(0, 0, mView.getWidth(), AndroidUtilities.lerp(y + AndroidUtilities.dp(BUTTONS_FADE_DP) / getScaleY() - 1, mView.getHeight(), 1f - progress), null);
